...
file_path_receiver = r'201_Domain_02_09\Weld_Region\small_correct_mesh\T32_201_Domain_improved_mesh.txt'  # Replace with your actual file path
...
plt.plot()
plt.plot(time_transducer[:300],amplitude_transducer[:300])
plt.show()

ref_sig = amplitude_transducer[:300]
...
for i in range(amplitude_receivers.shape[1]):
    ...
    # Adjust the length for correlation based on the current probe
    len_1 = np.argmax(amplitude_receivers[:, transmiter_probe]) + cutoff_len + a * (transmiter_probe - i)**2
    
    # Cross-correlate the reference signal with the current receiver's signal
    corr = signal.correlate(amplitude_receivers[:int(len_1), i], ref_sig, mode='full')
    lag = np.argmax(corr) - (len(ref_sig) - 1)
    
    # Calculate the peak time for the receiver
    peak_time_receiver = time_receiver[lag]
    
    # Compute the time delay
    time_delays[i] = np.abs(peak_time_receiver - peak_time_transducer)
    
    # Store the peak amplitude for the receiver
