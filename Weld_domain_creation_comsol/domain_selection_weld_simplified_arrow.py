...
def plot_arrow(ax, center, orientation, length, head_size=5, color='black'):
    ...
    arrow = patches.FancyArrowPatch((x, y), (x + dx, y + dy), 
                                    mutation_scale=head_size, color=color, 
                                    arrowstyle='->', linewidth=1.5)
    ax.add_patch(arrow)
    return arrow

def update_arrow(old_center, new_center, new_orientation, length, head_size=5):
    # Remove the old arrow if it exists
    if old_center in arrows:
        arrow = arrows[old_center]
        if arrow is not None:
            arrow.remove()  # Remove the old arrow
            del arrows[old_center]  # Remove reference from the dictionary
    
    # Plot the new arrow and store reference
    new_arrow = plot_arrow(ax, new_center, new_orientation, length, head_size=head_size)
    arrows[new_center] = new_arrow
    
    # Redraw the figure to include the new arrow
    ...
    if event.inaxes is not None:
        ...
        if distance <= width_step:  # Adjust the threshold as needed
            ...
            if orientation is not None:
                ...
                orientation_data.append([index + 1, nearest_center, (orientation)])  # Store data
                ...
                update_arrow(nearest_center, nearest_center, int(orientation), length=0.8)     
                ...
                if os.path.exists(file_path):
                    ...
                    # Remove duplicates, keeping the last entry for each "Square No."
                    updated_df = updated_df.sort_values(by='Square No.').drop_duplicates(subset=['Square No.'], keep='last')
                else:                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         
                    # Create new DataFrame
                    updated_df = pd.DataFrame(orientation_data, columns=['Square No.', 'Center Coordinates (mm)', 'Orientation (Degree)'])
                
                # Sort the DataFrame by 'Square No.' column
                updated_df = updated_df.sort_values(by='Square No.')
...
# Store references to arrows
arrows = {}  # Dictionary to store arrows with their centers as keys    
...
if os.path.exists(file_path):
    ...
    for idx, center in enumerate(centers):
        ...
        arrow = plot_arrow(ax, center, orientation, length=0.8, head_size=5)
        arrows[center] = arrow
